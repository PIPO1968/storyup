...
document.addEventListener('DOMContentLoaded', function () {
    // Mostrar chat en cualquier perfil (propio o ajeno), solo escribir si es amigo
    ...
    // Permitir responder en tu propio perfil si eres amigo del usuario mostrado (en este caso, tú mismo)
    if (!areFriends) {
        chatForm.querySelectorAll('input,button').forEach(el => { el.disabled = true; });
        chatInput.placeholder = 'Debes ser amigo para chatear';
        // Si es tu propio perfil, muestra los mensajes recibidos de amigos
        if (isOwnProfile) {
            renderChat();
        } else {
        ...
        // Si es tu propio perfil, permitir responder a amigos (a cada uno)
        if (isOwnProfile) {
            // Mostrar un selector de amigo para chatear
            let friends = getFriends(user.email).filter(f => f !== user.email);
            if (friends.length === 0) {
                chatForm.querySelectorAll('input,button').forEach(el => { el.disabled = true; });
                chatInput.placeholder = 'No tienes amigos para chatear';
                chatMessages.innerHTML = '<div style=\"color:#aaa;text-align:center;margin-top:2em;\">Agrega amigos para chatear aquí.</div>';
                return;
            }
            ...
            function updateChatForFriend(friendEmail) {
                ...
                // Redefinir renderChat para este amigo
                function renderChatFriend() {
                    const msgs = JSON.parse(localStorage.getItem(chatKey) || '[]');
                    chatMessages.innerHTML = msgs.map(m => {
                        if (m.type === 'img') {
                            return `<div style=\"margin-bottom:6px;text-align:${m.from === user.email ? 'right' : 'left'};\"><img src=\"${m.data}\" style=\"max-width:120px;max-height:120px;border-radius:8px;box-shadow:0 2px 8px #0003;display:inline-block;\"> <span style=\"font-size:0.8em;color:#aaa;\">${m.time}</span></div>`;
                ...
                // Emoji panel, imagen y video igual que antes (puedes reutilizar el código original)
                // ...
                // Recarga automática
                if (window.chatInterval) clearInterval(window.chatInterval);
                window.chatInterval = setInterval(renderChatFriend, 2000);
                renderChatFriend();
            }
            updateChatForFriend(selectedFriend);
            // Limpiar intervalo al salir
            window.addEventListener('beforeunload', () => clearInterval(window.chatInterval));
            return;
        }
        // Enviar mensaje texto
        chatForm.onsubmit = function (e) {
