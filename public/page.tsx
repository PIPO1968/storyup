...
import { UsersAPI, User as BaseUser } from "@/utils/users";
...
export default function Estadisticas() {
    ...
    async function calcularPuntosTotales(user: User | null): Promise<number> {
    ...
    const [tablaIndividual, setTablaIndividual] = React.useState<Record<string, unknown>>({});
    const [tablaCentro, setTablaCentro] = React.useState<Record<string, unknown>>({});
    const [tablaDocentes, setTablaDocentes] = React.useState<Record<string, unknown>>({});
    ...
    const [historiasArr, setHistoriasArr] = React.useState<Historia[]>([]);
    const [usersArr, setUsersArr] = React.useState<User[]>([]);
    React.useEffect(() => {
        setIsClient(true);
    ...
    React.useEffect(() => {
        const getTemporadasDisponibles = async () => {
            ...
            try {
                ...
                const temporadasSet = new Set(campeonatos.map((c: { temporada: number }) => c.temporada));
                temporadas.push(...Array.from(temporadasSet) as number[]);
            ...
            if (typeof window !== "undefined") {
                try {
                    ...
                    usersArr.forEach((u: User) => {
                        if (u.centro) {
                            const centroKey = String(u.centro);
                            if (!(centroKey in likesPorCentro)) likesPorCentro[centroKey] = 0;
                            likesPorCentro[centroKey] += Number(u.likes) || 0;
                        }
                        totalComp += Number(u.competicionesSuperadas) || 0;
                    });
                    ...
                    const docentesArr = usersArr.filter((u: User) => (u.tipo || "").toLowerCase() === "docente");
                    ...
                    const historiasGanadoras = historiasArr.filter((h: Historia) => h.likes && h.likes > 10).length;
                            ...
                            <div className="bg-white rounded-lg shadow-md p-6">
                                ...
                                <ul className="space-y-2">
                                    ...
                                    <li><span className="mr-2 text-xl">❓</span><b>Total de preguntas respondidas:</b> {(() => {
                                        ...
                                        let totalFalladas = 0;
                                        usersArr.forEach((u: User) => {
                                            totalAcertadas += Number(u.preguntasAcertadas) || 0;
                                            totalFalladas += Number(u.preguntasFalladas) || 0;
                                        });
                                    ...
                                    <li><span className="mr-2 text-xl">❓</span><b>{t('totalPreguntasAcertadas')}:</b> {(() => {
                                        let total = 0;
                                        usersArr.forEach((u: User) => {
                                            total += Number(u.preguntasAcertadas) || 0;
                                        });
                                        return total;
                                    })()}</li>
                                    <li><span className="mr-2 text-xl">❓</span><b>Total de preguntas falladas:</b> {(() => {
                                        let total = 0;
                                        usersArr.forEach((u: User) => {
                                            total += Number(u.preguntasFalladas) || 0;
                                        });
                                        return total;
                                    })()}</li>
                                    <li><span className="mr-2 text-xl">🏆</span><b>Total de concursos ganados:</b> {(() => {
                                        let total = 0;
                                        usersArr.forEach((u: User) => {
                                            total += Number(u.concursosGanados) || 0;
                                        });
                                        return total;
                                    })()}</li>
                                </ul>
                            ...
                            <div className="bg-white rounded-lg shadow-md p-6">
                                ...
                                <ul className="space-y-2">
                                    <li><span className="mr-2 text-xl">❤️</span><b>{t('usuariosMasLikes')}:</b> {(() => {
                                        const ranking = usersArr.map((u: User) => {
                                            return {
                                                nick: u.nick,
                                                nombre: u.nombre || '',
                                                totalLikes: Number(u.likes) || 0
                                            };
                                        })
                                    ...
                                    <li><span className="mr-2 text-xl">🤝</span><b>Usuarios con más amigos:</b> {(() => {
                                        const ranking = usersArr
                                            .map((u: User) => {
                                                const numAmigos = Array.isArray(u.amigos) ? u.amigos.length : 0;
                                                return {
                                                    nick: u.nick,
                                                    numAmigos
                                                };
                                    ...
                                    <li><span className="mr-2 text-xl">🏆</span><b>Usuarios con más concursos ganados:</b> {(() => {
                                        const ranking = usersArr.map((u: User) => ({
                                            nick: u.nick,
                                            nombre: u.nombre || '',
                                            totalConcursos: Number(u.concursosGanados) || 0
                                        }))
                                    ...
                                    <li><span className="mr-2 text-xl">🏆</span><b>Usuarios con más competiciones superadas:</b> {(() => {
                                        const ranking = usersArr
                                            .map((u: User) => ({
                                                nick: u.nick,
                                                nombre: u.nombre || '',
                                                totalCompeticiones: Number(u.competicionesSuperadas) || 0
                                            }))
                                        ...
                                        <ul className="space-y-2 text-sm">
                                            <li className="flex justify-between">
                                                ...
                                                <span className="font-bold text-blue-600">{(() => {
                                                    if (currentUser) {
                                                        const acertadas = Number(currentUser!.preguntasAcertadas) || 0;
                                                        const falladas = Number(currentUser!.preguntasFalladas) || 0;
                                            ...
                                            <li className="flex justify-between">
                                                ...
                                                <span className="font-bold text-blue-600">{String(currentUser?.racha ?? 0)} días</span>
                                            </li>
                                            <li className="flex justify-between">
                                            ...
                                            <li className="flex justify-between">
                                                ...
                                                <span className="font-bold text-blue-600">{(() => {
                                                    if (currentUser && usersArr.length > 0) {
                                                        const usersWithPoints = usersArr.map((u: User) => ({
                                                            ...u,
                                                            puntosTotales: 0 // Placeholder
                                                        }));
                                        ...
                                        <ul className="space-y-2 text-sm">
                                            <li className="flex justify-between">
                                                ...
                                                <span className="font-bold text-green-600">{(() => {
                                                    if (currentUser) {
                                                        const puntos = currentUser!.estadisticas?.matematicas || 0;
                                            ...
                                            <li className="flex justify-between">
                                                ...
                                                <span className="font-bold text-green-600">{(() => {
                                                    if (currentUser) {
                                                        const puntos = currentUser!.estadisticas?.lenguaje || 0;
                                                        const nivel = Math.floor(puntos / 5) + 1;
                                                        return `Nivel ${nivel}`;
                                                    }
                                                    return 'Nivel 1';
                                            ...
                                            <li className="flex justify-between">
                                                ...
                                                <span className="font-bold text-green-600">{(() => {
                                                    if (currentUser) {
                                                        const puntos = currentUser!.estadisticas?.geografia || 0;
                                                        const nivel = Math.floor(puntos / 5) + 1;
                                                        return `Nivel ${nivel}`;
                                                    }
                                                    return 'Nivel 1';
                                            ...
                                            <li className="flex justify-between">
                                                ...
                                                <span className="font-bold text-green-600">{(() => {
                                                    if (currentUser) {
                                                        const puntos = currentUser!.estadisticas?.historia || 0;
                                                        const nivel = Math.floor(puntos / 5) + 1;
                                                        return `Nivel ${nivel}`;
                                                    }
                                                    return 'Nivel 1';
                                            ...
                                            <li className="flex justify-between">
                                                ...
                                                <span className="font-bold text-green-600">{(() => {
                                                    if (currentUser) {
                                                        const asignaturas = [
                                                            { nombre: 'Matemáticas', puntos: currentUser!.estadisticas?.matematicas || 0 },
                                                            { nombre: 'Lenguaje', puntos: currentUser!.estadisticas?.lenguaje || 0 },
                                                            { nombre: 'Geografía', puntos: currentUser!.estadisticas?.geografia || 0 },
                                                            { nombre: 'Historia', puntos: currentUser!.estadisticas?.historia || 0 },
                                                            { nombre: 'Literatura', puntos: currentUser!.estadisticas?.literatura || 0 },
                                                            { nombre: 'Inglés', puntos: currentUser!.estadisticas?.ingles || 0 },
                                                            { nombre: 'Naturaleza', puntos: currentUser!.estadisticas?.naturaleza || 0 }
                                                        ];
                                            ...
                                            <li className="flex justify-between">
                                                ...
                                                <span className="font-bold text-purple-600">{(() => {
                                                    if (currentUser) {
                                                        ...
                                                        const puntosActuales = Number(currentUser.likes) || 0;
                                ...
                                <div className="mt-6 bg-gradient-to-r from-indigo-50 to-purple-50 rounded-xl p-4">
                                    ...
                                    <div className="text-center mt-3 text-sm text-indigo-600">
                                        📈 <strong>Tendencia:</strong> {(() => {
                                            if (currentUser) {
                                                const acertadas = (Number(currentUser.preguntasAcertadas) || 0) + parseInt(localStorage.getItem('acertadas_' + currentUser.nick) || '0', 10);
                                                const falladas = (Number(currentUser.preguntasFalladas) || 0) + parseInt(localStorage.getItem('falladas_' + currentUser.nick) || '0', 10);
                                    ...
                                    <div className="grid grid-cols-3 gap-4 text-center">
                                        ...
                                        <div className="bg-gradient-to-r from-blue-100 to-purple-100 rounded-lg p-3 border-2 border-blue-300">
                                            ...
                                            <div className="text-xs text-blue-500">{(() => {
                                                if (typeof window !== "undefined" && currentUser) {
                                                    ...
                                                    if (users.length > 0) {
                                                        const usersWithPoints = users.map((u: User) => ({
                                                            ...u,
                                                            puntosTotales: calcularPuntosTotales(u)
                                                        }));
                                                        const sorted = usersWithPoints.sort((a: any, b: any) => b.puntosTotales - a.puntosTotales);
                                        ...
                                        <div className="bg-white rounded-lg p-3">
                                            ...
                                            <div className="font-bold text-gray-600">{(() => {
                                                if (usersArr.length > 0) {
                                                    // Simplified synchronous calculation
                                                    const puntosArr = usersArr.map((u: User) => (u.likes as number || 0));
                                                    const total = puntosArr.reduce((sum: number, puntos: number) => sum + puntos, 0);
                                    ...
                                    <div className="mt-3 text-center text-sm">
                                        {(() => {
                                            if (currentUser && usersArr.length > 0) {
                                                ...
                                                const tusPuntos = Number(currentUser.likes) || 0;
                ...
                <ul className="space-y-2 text-center">
                    <li><span className="mr-2 text-xl">📖</span><b>Historia con más likes:</b> {
                        (() => {
                            ...
                            const top = historiasArr.reduce((max: Historia, h: Historia) => {
                    ...
                    <li><span className="mr-2 text-xl">🏫</span><b>Centro con más historias creadas:</b> {(() => {
                        ...
                        usersArr.forEach((u: User) => {
                            if (u.nick && u.centro) {
                                nickToCentro[u.nick] = u.centro as string;
                            }
                        });
                    ...
                    <li><span className="mr-2 text-xl">🏫</span><b>Centro Escolar con más usuarios activos:</b> {(() => {
                        if (typeof window !== "undefined") {
                            ...
                            if (usersStr) {
                                try {
                                    const usersArr: User[] = JSON.parse(usersStr || "[]");
                                    if (usersArr.length === 0) return "-";
                                    const usuariosPorCentro: Record<string, number> = {};
                                    usersArr.forEach((u: User) => {
                                        if (u.centro) {
                                            usuariosPorCentro[u.centro as string] = (usuariosPorCentro[u.centro as string] || 0) + 1;
                                        }
                                    });
