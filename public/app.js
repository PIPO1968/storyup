...
form.addEventListener('submit', (e) => {
    ...
    if (type === 'nick' && user && user.displayName) {
    ...
    form.reset();
    messageDiv.textContent = '¡Historia publicada!';
});



function loadStories(languageFilter) {
    db.ref('stories').once('value', (snapshot) => {
        feed.innerHTML = '';
        snapshot.forEach((childSnapshot) => {
            const story = childSnapshot.val();
            if (languageFilter === 'all' || story.language === languageFilter) {
                const div = document.createElement('div');
                div.classList.add('story');
                div.innerHTML = `
	<h3>${story.title}</h3>
