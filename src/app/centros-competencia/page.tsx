...
export default function CentrosCompetencia() {
    ...
    const obtenerActividadCompeticiones = async (centro: string): Promise<number> => {
        ...
        try {
            ...
            const responseCampeonato = await fetch(`/api/campeonatos?temporada=t${temporadaActual}&tipo=individual`);
            ...
            const responseDocentes = await fetch(`/api/campeonatos?temporada=t${temporadaActual}&tipo=docentes`);
            const campeonatoDocentes = await responseDocentes.json();
    ...
    const obtenerEstadisticasAsignatura = async (asignatura: string, usuarios: Usuario[], cursoFiltro?: string): Promise<{ [centro: string]: number }> => {
        ...
        for (const usuario of usuarios) {
            ...
            const asignaturaParaAPI = mapaAsignaturasInverso[asignatura] || asignatura.toLowerCase();
            const response = await fetch(`/api/estadisticas?nick=${usuario.nick}&tipo=${asignaturaParaAPI}`);
            const estadisticas = await response.json();
            const puntos = estadisticas.length > 0 ? estadisticas[0].puntos : 0;

            if (puntos > 0) {
                // Solo usar datos reales específicos de la asignatura
                estadisticasCentros[centro] += puntos;
            }
        }
    ...
    const calcularStatsDelCentro = async (nombreCentro: string, usuarios: Usuario[]): Promise<CentroStats> => {
        ...
        const puntosCompeticiones = await obtenerActividadCompeticiones(nombreCentro);
    ...
    const obtenerPremiosDelMes = async (): Promise<any[]> => {
        ...
        const responseAnterior = await fetch(`/api/premios-mensuales?mes=${mesAnterior}&anio=${añoAnterior}`);
        const premiosAnteriores = await responseAnterior.json();
        if (premiosAnteriores.length === 0) {
            const premiosGenerados = generarPremiosAutomaticos(mesAnterior, añoAnterior);
            if (premiosGenerados.length > 0) {
                try {
                    for (const premio of premiosGenerados) {
                        await fetch('/api/premios-mensuales', {
                            method: 'POST',
        ...
        const responseActual = await fetch(`/api/premios-mensuales?mes=${mesActual}&anio=${añoActual}`);
        const premiosGuardados = await responseActual.json();

        // Normalizar premios (evitar que el campo `premio` sea un objeto)
        const normalized = premiosGuardados.map((p: any) => {
            const prem = p?.premio;
        ...
        if (needsMigration && !localStorage.getItem(`${clavePremiosActual}_migrated_v1`)) {
            try {
                sessionStorage.setItem(clavePremiosActual, JSON.stringify(normalized));
...
const asignarTrofeoACentro = async (nombreCentro: string, idTrofeo: number): Promise<void> => {
    if (typeof window === "undefined") return;
...
const trofeosYaAsignadosEsteMes = (): boolean => {
    ...
    return sessionStorage.getItem(claveTrofeosAsignados) === 'true';
};
...
const marcarTrofeosAsignadosEsteMes = (): void => {
    ...
    sessionStorage.setItem(claveTrofeosAsignados, 'true');
};
...
const obtenerMesesDisponibles = (): string[] => {
    ...
    for (let i = 1; i <= 12; i++) {
        ...
        if (sessionStorage.getItem(claveMes)) {
...
const guardarRankingMensual = (centros: CentroStats[]) => {
    ...
    sessionStorage.setItem(claveMes, JSON.stringify(datosRanking));
};
...
const cargarRankingMensual = (año: number, mes: number): CentroStats[] => {
    ...
    const datos = sessionStorage.getItem(claveMes);
    ...
    const cargarDatos = async () => {
        try {
            ...
            if (asignaturaSeleccionada !== "todas") {
                ...
                const asignaturaParaAPI = mapaAsignaturasInverso[asignaturaSeleccionada] || asignaturaSeleccionada.toLowerCase();

                let hayDatosEspecificos = false;
                const response = await fetch(`/api/estadisticas?tipo=${asignaturaParaAPI}`);
                const estadisticas = await response.json();
                if (estadisticas.length > 0) {
                    hayDatosEspecificos = estadisticas.some((e: any) => e.puntos > 0);
                }
...
if (asignaturaSeleccionada === "todas" && cursoSeleccionado === "todos") {
    ...
    descripcionTitulo = `Vista específica de ${cursoSeleccionado} - ${mesActual}`;
} else {
    descripcionTitulo = hayDatosAsignatura
                    ...
                    <table className="w-full">
                        ...
                        <tbody className="bg-white divide-y divide-gray-200">
                            {centros.map((centro, index) => (
                                <tr
                                    key={centro.nombre}
                                    className={`hover:bg-gray-50 ${centro.nombre === miCentro
                    ...
                    {premiosDelMes.length > 0 ? (
                        <div className="space-y-2">
                            {premiosDelMes.slice(0, 10).map((premio, index) => (
                                <div
                                    key={index}
                                    className={`flex items-center justify-between p-2 rounded ${premio.centro === miCentro
